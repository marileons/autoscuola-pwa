...
test("quota esaurita lascia invariati creazione modifica e guida nello stato osservabile",async()=>{const extract=name=>source.match(new RegExp(`(?:async )?function ${name}\\([^\\n]+\\n`))?.[0]||source.match(new RegExp(`(?:async )?function ${name}\\([^}]+\\}`))?.[0]||"";const code=["storageStudents","friendlyStudentSaveError","save"].map(extract).join("\n"),previous=[{id:"a",firstName:"ANNA",lessons:[{id:"g1",notes:"prima"}],photo:"data:image/png;base64,AAAA"}],alerts=[],sandbox={state:{students:JSON.parse(JSON.stringify(previous))},committedStudents:JSON.parse(JSON.stringify(previous)),normalizeStudent:value=>JSON.parse(JSON.stringify(value)),requireWritableArchive:()=>{},KEY:"autoscuola_v3_completa",alert:value=>alerts.push(value),studentSaving:false,studentArchiveStore:{async replaceAll(){throw Object.assign(Error("quota"),{name:"QuotaExceededError"})}}};vm.runInNewContext(`${code};globalThis.run=save`,sandbox);for(const mutation of [students=>students.push({id:"b",lessons:[]}),students=>students[0].firstName="CAMBIATA",students=>students[0].lessons[0].notes="dopo"]){sandbox.state.students=JSON.parse(JSON.stringify(previous));mutation(sandbox.state.students);await assert.rejects(()=>sandbox.run(),/Spazio locale insufficiente/);assert.equal(JSON.stringify(sandbox.state.students),JSON.stringify(previous))}assert.ok(alerts.every(message=>/dati precedenti sono rimasti invariati/.test(message)))});
