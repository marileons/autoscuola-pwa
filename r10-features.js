...
(()=>{
  ...
  function studentReportHtml(current){
    const generated=new Date(),significant=(current.checklist||[]).filter(item=>item.status!=="none");
    const checklistRows=(significant.length?significant:current.checklist||[]).map(item=>`<tr><td>${esc(item.label)}</td><td>${statusLabel(item.status)}</td></tr>`).join("")||'<tr><td colspan="2">Nessuna voce disponibile</td></tr>';
    const lessons=[...(current.lessons||[])].sort((a,b)=>a.createdAt-b.createdAt).map((item,index)=>{
      ...
      const metres=routeDistance(item.route||[]),duration=routeDuration(item.route||[]);
      return`<section class="lesson"><h3>Guida ${index+1} · ${new Date(item.createdAt).toLocaleString("it-IT")}</h3><dl><div><dt>Durata GPS</dt><dd>${formatDuration(duration)}</dd></div><div><dt>Distanza GPS</dt><dd>${item.route&&item.route.length>1?formatDistance(metres):"GPS non usato"}</dd></div></dl><p><strong>Note:</strong> ${esc(item.notes||"Nessuna nota")}</p><ul>${selected}</ul></section>`;
