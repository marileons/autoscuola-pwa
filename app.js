...
function selectStudentSection(category){state.filter=category;document.querySelectorAll(".categories button[data-f]").forEach(button=>button.classList.toggle("active",button.dataset.f===category));$("newStudent").disabled=category===UNCLASSIFIED_CATEGORY;renderStudents()}
...
function drawLive(){initLiveMap();if(state.liveLine)state.liveMap.removeLayer(state.liveLine);if(state.liveMarker)state.liveMap.removeLayer(state.liveMarker);state.liveLine=null;state.liveMarker=null;if(!state.tempRoute.length)return;const segments=routeSegments(state.tempRoute),layers=segments.filter(x=>x.length>1).map(x=>L.polyline(x,{weight:5}));state.liveLine=L.featureGroup(layers).addTo(state.liveMap);const last=state.tempRoute.at(-1);state.liveMarker=L.marker([last.lat,last.lng]).addTo(state.liveMap);if(state.tempRoute.length===1)state.liveMap.setView([last.lat,last.lng],17);else{const bounds=state.liveLine.getBounds();if(bounds.isValid())state.liveMap.fitBounds(bounds,{padding:[20,20]});else state.liveMap.setView([last.lat,last.lng],17)}}
function startGps(){if(!navigator.geolocation||state.watch!==null)return;$("gpsStatus").textContent="GPS in acquisizione…";$("startGps").disabled=true;$("stopGps").disabled=false;state.watch=navigator.geolocation.watchPosition(pos=>{const p={lat:pos.coords.latitude,lng:pos.coords.longitude,accuracy:Number(pos.coords.accuracy||999),time:pos.timestamp,breakBefore:false};if(p.accuracy>70){$("gpsStatus").textContent="GPS attivo · attendo un segnale più preciso";return}const prev=state.tempRoute.at(-1);if(!prev){state.tempRoute.push(p);drawLive()}else{const metres=distance(prev,p),hasTime=Number.isFinite(prev.time),seconds=hasTime?Math.max(0.001,(p.time-prev.time)/1000):0,speed=hasTime?metres/seconds:0;if(metres>=5){p.breakBefore=!hasTime&&metres>80||hasTime&&seconds>15&&metres>80||metres>300||hasTime&&speed>55;state.tempRoute.push(p);drawLive()}}$("gpsStatus").textContent=`GPS attivo · ${state.tempRoute.length} punti`},err=>{if(err.code===1){$("gpsStatus").textContent="Permesso GPS negato.";stopGps();return}$("gpsStatus").textContent=err.code===2?"Segnale GPS temporaneamente non disponibile · registrazione ancora attiva":"Tempo di acquisizione GPS scaduto · registrazione ancora attiva"},{enableHighAccuracy:true,maximumAge:0,timeout:15000})}
