...
let state={students:load(),examiners:loadExaminers(),studentId:null,lessonId:null,editingStudent:null,examinerId:null,editingExaminer:null,habitDraft:[],filter:null,managerType:"auto",lessonsExpanded:false,watch:null,tempRoute:[],gpsNeedsBreak:false,liveMap:null,liveLine:null,liveMarker:null,savedMap:null,savedLine:null,recognition:null,recognizing:false};
...
function setNeutralHome(){state.filter=null;document.querySelectorAll(".categories button[data-f]").forEach(button=>button.classList.remove("active"));$("studentSection").classList.add("hidden");$("unclassifiedStudents").classList.add("hidden")}
function show(id){document.querySelectorAll(".view").forEach(v=>v.classList.remove("active"));$(id).classList.add("active");if(id==="home")setNeutralHome();scrollTo(0,0)}
...
function renderStudents(){const q=$("search").value.trim().toLowerCase(),a=state.students.filter(s=>!s.archived&&s.category===state.filter).filter(s=>(nameOf(s)+" "+s.phone+" "+s.license).toLowerCase().includes(q)).sort((a,b)=>nameOf(a).localeCompare(nameOf(b),"it"));$("currentSectionTitle").textContent=sectionLabel(state.filter);$("students").innerHTML="";$("emptyStudents").classList.toggle("hidden",a.length>0);a.forEach(s=>{const b=document.createElement("button");b.className="student-card";b.innerHTML=`<strong>${esc(nameOf(s)||"Senza nome")}</strong><span class="meta">${sectionLabel(s.category)} · ${esc(s.license||"Patente non indicata")} · ${s.lessons.length} guide</span>`;b.onclick=()=>openStudent(s.id);$("students").appendChild(b)});const needsClassification=state.students.some(s=>!s.archived&&s.category===UNCLASSIFIED_CATEGORY),button=$("unclassifiedStudents");button.classList.toggle("hidden",!needsClassification);button.classList.toggle("active",state.filter===UNCLASSIFIED_CATEGORY)}
function renderArchivedStudents(){const q=$("archiveSearch").value.trim().toLowerCase(),items=state.students.filter(s=>s.archived).filter(s=>(nameOf(s)+" "+s.phone+" "+s.license+" "+sectionLabel(s.category)).toLowerCase().includes(q)).sort((a,b)=>nameOf(a).localeCompare(nameOf(b),"it"));$("archivedStudents").replaceChildren();$("emptyArchive").classList.toggle("hidden",items.length>0);items.forEach(s=>{const b=document.createElement("button");b.className="student-card archived-student";b.innerHTML=`<strong>${esc(nameOf(s)||"Senza nome")}</strong><span class="meta">${sectionLabel(s.category)} · ${esc(s.license||"Patente non indicata")} · ${s.lessons.length} guide</span>`;b.onclick=()=>openStudent(s.id);$("archivedStudents").appendChild(b)})}
function selectStudentSection(category){state.filter=category;$("studentSection").classList.remove("hidden");document.querySelectorAll(".categories button[data-f]").forEach(button=>button.classList.toggle("active",button.dataset.f===category));$("newStudent").disabled=category===UNCLASSIFIED_CATEGORY;renderStudents();$("studentSection").scrollIntoView({behavior:"smooth",block:"start"})}
...
function archiveStudent(){const s=student();if(!s||s.archived)return;if(confirm(`Archiviare ${nameOf(s)||"questo allievo"}? Tutti i dati saranno conservati.`)){s.archived=true;save();renderStudents();show("home")}}
...
function evaluateGpsPoint(prev,p,pendingBreak=false){if(!prev)return{accept:true,breakBefore:false,reason:"first"};const metres=distance(prev,p),seconds=Number.isFinite(prev.time)?(p.time-prev.time)/1000:null;if(seconds!==null&&seconds<=0)return{accept:false,breakBefore:false,reason:"stale",metres,seconds};const speed=seconds&&seconds>0?metres/seconds:null,minMove=Math.max(3,Math.min(8,(prev.accuracy+p.accuracy)*.08));if(metres<minMove)return{accept:false,breakBefore:false,reason:"duplicate",metres,seconds,speed};if(speed!==null&&speed>65)return{accept:false,breakBefore:false,reason:"impossible",metres,seconds,speed};const uncertain=Math.max(150,(prev.accuracy+p.accuracy)*2),longGap=seconds!==null&&seconds>60&&metres>uncertain,wideGap=seconds!==null&&seconds>30&&metres>1000,poorRecovery=pendingBreak&&seconds!==null&&seconds>20&&metres>Math.max(200,uncertain);return{accept:true,breakBefore:longGap||wideGap||poorRecovery,reason:longGap||wideGap||poorRecovery?"gap":"track",metres,seconds,speed}}
function startGps(){if(state.watch!==null){stopGps();return}if(!navigator.geolocation){updateGpsUi("Geolocalizzazione non supportata da questo dispositivo o browser.");return}state.gpsNeedsBreak=false;try{const watchId=navigator.geolocation.watchPosition(pos=>{const lat=Number(pos.coords.latitude),lng=Number(pos.coords.longitude),accuracy=Number(pos.coords.accuracy),time=Number(pos.timestamp)||Date.now();if(!Number.isFinite(lat)||!Number.isFinite(lng)||!Number.isFinite(accuracy)){state.gpsNeedsBreak=true;updateGpsUi("Posizione GPS ricevuta ma non valida.");return}if(accuracy>100){state.gpsNeedsBreak=true;updateGpsUi("GPS attivo · attendo un segnale più preciso");return}const p={lat,lng,accuracy,time,breakBefore:false},decision=evaluateGpsPoint(state.tempRoute.at(-1),p,state.gpsNeedsBreak);if(decision.accept){p.breakBefore=decision.breakBefore;state.tempRoute.push(p);state.gpsNeedsBreak=false;drawLive()}else if(decision.reason==="impossible"){state.gpsNeedsBreak=true;updateGpsUi("GPS attivo · salto anomalo ignorato");return}updateGpsUi(`GPS attivo · ${state.tempRoute.length} punti registrati${p.breakBefore?" · nuovo segmento":""}`)},error=>{const message=gpsErrorMessage(error);if(error.code===1)stopGps(message);else{state.gpsNeedsBreak=true;updateGpsUi(`${message} Registrazione ancora attiva.`)}},{enableHighAccuracy:true,maximumAge:0,timeout:20000});state.watch=watchId;updateGpsUi("GPS attivo · acquisizione della posizione…")}catch(error){state.watch=null;updateGpsUi("Impossibile avviare il GPS su questo dispositivo.")}}
function stopGps(message){if(state.watch!==null&&navigator.geolocation)navigator.geolocation.clearWatch(state.watch);state.watch=null;updateGpsUi(message||"GPS non attivo.")}function toggleGps(){if(state.watch===null)startGps();else stopGps()}function resetGps(){stopGps(state.tempRoute.length?`Percorso presente · ${state.tempRoute.length} punti`:"GPS non attivo.");drawLive()}
...
$("unclassifiedStudents").onclick=()=>selectStudentSection(UNCLASSIFIED_CATEGORY);
$("archiveSearch").oninput=renderArchivedStudents;
$("studentSectionHome").onclick=()=>show("home");
