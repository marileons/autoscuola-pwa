...
function renderStudents(){const q=$("search").value.trim().toLowerCase(),a=state.students.filter(s=>s.category===state.filter).filter(s=>(nameOf(s)+" "+s.phone+" "+s.license).toLowerCase().includes(q)).sort((a,b)=>nameOf(a).localeCompare(nameOf(b),"it"));$("currentSectionTitle").textContent=sectionLabel(state.filter);$("students").innerHTML="";$("emptyStudents").classList.toggle("hidden",a.length>0);a.forEach(s=>{const b=document.createElement("button");b.className="student-card";b.innerHTML=`<strong>${esc(nameOf(s)||"Senza nome")}</strong><span class="meta">${sectionLabel(s.category)} · ${esc(s.license||"Patente non indicata")} · ${s.lessons.length} guide</span>`;b.onclick=()=>openStudent(s.id);$("students").appendChild(b)});const needsClassification=state.students.some(s=>s.category===UNCLASSIFIED_CATEGORY),button=$("unclassifiedStudents");button.classList.toggle("hidden",!needsClassification);button.classList.toggle("active",state.filter===UNCLASSIFIED_CATEGORY)}
