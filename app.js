...
function startGps(){if(state.watch!==null){stopGps();return}if(!navigator.geolocation){updateGpsUi("Geolocalizzazione non supportata da questo dispositivo o browser.");return}try{const watchId=navigator.geolocation.watchPosition(pos=>{const lat=Number(pos.coords.latitude),lng=Number(pos.coords.longitude),accuracy=Number(pos.coords.accuracy),time=Number(pos.timestamp)||Date.now();if(!Number.isFinite(lat)||!Number.isFinite(lng)||!Number.isFinite(accuracy)){updateGpsUi("Posizione GPS ricevuta ma non valida.");return}if(accuracy>100){updateGpsUi("GPS attivo · attendo un segnale più preciso");return}const p={lat,lng,accuracy,time,breakBefore:false},prev=state.tempRoute.at(-1);if(!prev){state.tempRoute.push(p);drawLive()}else{const metres=distance(prev,p),seconds=Number.isFinite(prev.time)?Math.max(.001,(p.time-prev.time)/1000):null,speed=seconds===null?null:metres/seconds,minMove=Math.max(3,Math.min(8,(prev.accuracy+p.accuracy)*.08));if(metres>=minMove){p.breakBefore=metres>300&&(speed===null||speed>65);state.tempRoute.push(p);drawLive()}}updateGpsUi(`GPS attivo · ${state.tempRoute.length} punti registrati`)},error=>{const message=gpsErrorMessage(error);if(error.code===1)stopGps(message);else updateGpsUi(`${message} Registrazione ancora attiva.`)},{enableHighAccuracy:true,maximumAge:0,timeout:20000});state.watch=watchId;updateGpsUi("GPS attivo · acquisizione della posizione…")}catch(error){state.watch=null;updateGpsUi("Impossibile avviare il GPS su questo dispositivo.")}}
