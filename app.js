...
function renderDrivingErrors(){const box=$("drivingErrorsList"),count=state.tempErrors.length,pending=state.tempErrors.filter(item=>item.status==="pending").length;$("drivingErrorCount").textContent=`Errori registrati: ${count}`;$("drivingErrorsSummary").textContent=`ERRORI SEGNALATI DURANTE LA GUIDA (${count})`;box.replaceChildren();$("drivingErrorsEmpty").classList.toggle("hidden",count>0);state.tempErrors.forEach((item,index)=>{const card=document.createElement("article");card.className="driving-error-card";const editing=state.editingErrorId===item.id;if(editing){const options=[{id:"unclassified",label:"Da classificare"},...DRIVING_ERRORS.CATEGORIES].map(category=>`<option value="${category.id}" ${category.id===item.category?"selected":""}>${esc(category.label)}</option>`).join("");card.innerHTML=`<strong>Errore ${index+1} · ${new Date(item.occurredAt).toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"})}</strong><label>Categoria<select class="driving-error-category">${options}</select></label><label>Nota<textarea class="driving-error-note" maxlength="500" rows="3">${esc(item.note)}</textarea></label><div class="driving-error-actions"><button type="button" class="save-error">SALVA</button><button type="button" class="secondary cancel-error">ANNULLA</button></div>`;card.querySelector(".save-error").onclick=()=>saveDrivingErrorEdit(item.id,card);card.querySelector(".cancel-error").onclick=()=>{state.editingErrorId=null;renderDrivingErrors()}}else{card.innerHTML=`<strong>Errore ${index+1} · ${new Date(item.occurredAt).toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"})}</strong><span>${esc(DRIVING_ERRORS.categoryLabel(item.category))} · ${esc(drivingErrorLocationLabel(item))}</span><p>${esc(item.note||"Nessuna nota")}</p><div class="driving-error-actions"><button type="button" class="secondary edit-error">MODIFICA</button><button type="button" class="danger delete-error">ELIMINA</button></div>`;card.querySelector(".edit-error").onclick=()=>{state.editingErrorId=item.id;renderDrivingErrors()};card.querySelector(".delete-error").onclick=()=>deleteDrivingError(item.id)}box.appendChild(card)});$("drivingErrorsPending").textContent=pending?`${pending} da classificare`:count?"Tutti classificati":"";drawDrivingErrorMarkers(state.liveMap,"liveErrorLayer",state.tempErrors)}
function saveDrivingErrorEdit(id,card){const index=state.tempErrors.findIndex(item=>item.id===id);if(index<0)return;const category=card.querySelector(".driving-error-category").value,note=card.querySelector(".driving-error-note").value;state.tempErrors[index]=DRIVING_ERRORS.normalizeError({...state.tempErrors[index],category,status:category==="unclassified"?"pending":"completed",note,updatedAt:Date.now()},index);state.editingErrorId=null;persistDrivingErrorDraft();renderDrivingErrors()}
...
function drawDrivingErrorMarkers(map,stateKey,items){if(!map||typeof L==="undefined")return;if(state[stateKey])map.removeLayer(state[stateKey]);const markers=DRIVING_ERRORS.normalizeErrors(items).map((item,index)=>item.locationStatus==="available"?L.marker([item.lat,item.lng],{icon:L.divIcon({className:"driving-error-marker",html:String(index+1),iconSize:[30,30],iconAnchor:[15,15]})}).bindPopup(`<strong>Errore ${index+1}</strong><br>${esc(new Date(item.occurredAt).toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"}))}<br>${esc(DRIVING_ERRORS.categoryLabel(item.category))}${item.note?`<br>${esc(item.note)}`:""}`):null).filter(Boolean);state[stateKey]=markers.length?L.featureGroup(markers).addTo(map):null}
